...
class DateParser(object):
    ...
    def parse_date(date):
        ...
        if type(date) is not str:
            raise ValueError('parseDate can\'t handle type except str current type is %s  ' % type(date))
    ...
    def parse_date_time(date):
        if date is None:
            raise ValueError('date can\'t be None')
        if type(date) is not str:
            raise ValueError('parse_date_time can\'t handle type except str current type is %s  ' % type(date))
...
class DateOperator(object):
    ...
    def add_hours(start_date, num):
        if type(start_date) is not datetime.datetime:
            raise ValueError('add_hours can\'t handle type  %s  ' % type(start_date))
    ...
    def add_days(start_date, num):
        if type(start_date) is not datetime.datetime:
            raise ValueError('add_days can\'t handle type  %s  ' % type(start_date))
        if type(num) is not int:
            raise ValueError('add_days can\'t handle type  %s  ' % type(start_date))
    ...
    def add_weeks(start_date, num):
        if type(start_date) is not datetime.datetime:
            raise ValueError('add_weeks can\'t handle type  %s  ' % type(start_date))
        if type(num) is not int:
            raise ValueError('add_weeks can\'t handle type  %s  ' % type(start_date))
    ...
    def add_months(start_date, num):
        if type(start_date) is not datetime.datetime:
            raise ValueError('add_months can\'t handle type  %s  ' % type(start_date))
        if type(num) is not int:
            raise ValueError('add_months can\'t handle type  %s  ' % type(start_date))
